...
![Version Backend](https://img.shields.io/badge/Backend-v2.8.31-blue)
![Version Frontend](https://img.shields.io/badge/Frontend-v2.13.63-blue)
...
| 2.13.62 | Erträge-Chart: Jahressumme unter den Monatsbalken ergänzt — zeigt Dividenden und realisierte Gewinne/Verluste getrennt für das im Jahres-Umschalter gewählte Jahr (💵/📈, farbcodiert grün/rot je nach Vorzeichen). Folgt derselben gefilterten Basis wie die Balken selbst (Typ-Filter, Suche, gewähltes Jahr) — bei aktivem Typ-Filter erscheint nur die jeweils relevante Zeile |
| 2.13.60–61 | System-Status-Modal: Button „✕ Leeren" bei „Letzte Fehler" ergänzt (nur sichtbar, wenn Einträge vorhanden) — ruft den bereits länger bestehenden Backend-Endpoint `POST /api/health/clear-errors` auf und lädt das Modal danach neu; der Endpoint war zwar dokumentiert, aber im Frontend nie angebunden. Bugfix (v2.13.61): der Footer-Statusdot bestimmte den Rot/Grün-Zustand entgegen der Doku weiterhin über die Länge des Fehler-Logs statt über das dafür vorgesehene `last_refresh_had_errors`-Flag — blieb dadurch bis zum manuellen Leeren rot, auch nach einem bereits fehlerfreien Refresh-Zyklus. Liest jetzt korrekt das Flag |
| 2.13.58–59 | Erträge-Modal: neue Monatsübersicht als Balkenchart (Jan–Dez, Jahres-Umschalter ‹ › mit oberer Grenze = laufendes Kalenderjahr, untere Grenze = Jahr des ältesten Erträge-Eintrags) — Dividenden grün, realisierte Gewinne/Verluste blau, Verluste unter der Nulllinie in Rot. Reagiert auf den bestehenden Typ-Filter (Alle/Dividenden/Verkäufe) und die Aktien-Suche, genau wie die Liste darunter. Rein frontend-seitige Aggregation der bereits geladenen Erträge-Daten, kein neuer Backend-Endpoint. Bugfix (v2.13.59): auf Touch-Geräten (z.B. iPad) verschwand der Tooltip beim Antippen eines Balkens sofort wieder — lag am ursprünglich genutzten Press-and-Hold-Muster (analog zum Crosshair im Portfolio-Verlauf-Chart), ungeeignet für diskrete Balken. Umgestellt auf Tap-Toggle: bleibt stehen bis ein anderer Balken oder wo anders im Modal getippt wird |
| 2.8.30 / 2.13.57 | Erträge-Modal: Dividenden und realisierte Gewinne/Verluste zeigen jetzt den tatsächlich zugeflossenen Netto-Betrag (nach Steuer/Gebühr, von Parqet als `amountNet`/`realizedGainsNet` mitgeliefert) statt brutto — sowohl in Zeilenanzeige, Summenkachel als auch Suche. Bereits gespeicherte Dividenden-Alt-Einträge ohne Netto-Feld werden beim nächsten Sync automatisch nachgerechnet. Versionssprung bereinigt eine Kollision zwischen zwei parallel entstandenen Änderungssträngen (Erträge-Feature und Rebalancing hatten unabhängig überlappende Versionsnummern vergeben) — Rebalancing ist davon inhaltlich nicht betroffen und vollständig enthalten |
...
| 2.8.25 / 2.13.52 | Parqet-Sync erfasst jetzt zusätzlich `dividend`-Aktivitäten und wertet `realizedGains`/`realizedGainsNet` bei Verkäufen aus (beides von Parqet selbst berechnet, bisher komplett verworfen). Neue Dateien `realized_gains.json`/`dividends.json`, neuer Read-Endpoint `GET /api/depots/<id>/earnings`. Rein additiv — bestehende Sync-Logik (Holdings, ISIN-Abgleich, `removed[]`) unverändert. Datenquelle ausschließlich Parqet, keine manuelle Erfassung. Neue „💰 Erträge gesamt"-Kachel in der Portfolio-Übersicht (nur sichtbar bei vorhandenen Daten) öffnet ein Erträge-Modal mit Typ-Filter (Alle/Dividenden/Verkäufe) und Aktien-Suche, die die Summenkacheln live umrechnet; zeigt auch komplett verkaufte, aus dem Bestand entfernte Positionen |
| 2.8.24 / 2.13.50 | Parqet-Sync jetzt auch direkt in der Hauptansicht möglich (nicht mehr nur über Depot-Einstellungen): neuer „🔗 Parqet Sync"-Button in der Werkzeugleiste plus „Parqet: zuletzt …"-Info in der Kopfzeile, beides nur sichtbar wenn das aktive Depot verbunden ist. Bleibt automatisch mit dem Depot-Einstellungen-Modal synchron. Sicherheits-Härtung: `GET /api/depots` lieferte bisher ungefiltert auch Parqet Access-/Refresh-Token an das Frontend aus, obwohl dort nirgends gelesen — werden jetzt vor der Auslieferung entfernt |
| 2.8.23 | Tatsächliche Ursache für Parqet-Reconnects gefunden und behoben (korrigiert die ~2h-Theorie aus v2.8.22, die sich als falsch herausstellte): laut offizieller Parqet-Auskunft sind Refresh Tokens Single-Use und laufen erst nach 60 Tagen Inaktivität ab. Der reale Bug lag in `parqet_sync()` — ein am Funktionsanfang geladener, veralteter Depot-Stand wurde am Ende erneut zum Speichern verwendet und überschrieb dabei einen zwischenzeitlich frisch rotierten Refresh Token wieder mit dem bereits verbrauchten alten (klassisches „stale object"-Muster). Fix: der letzte Speichervorgang lädt jetzt selbst frisch von der Platte |
...
| 2.8.14–15 / 2.13.25–27 | Anzahl und Einstandskurs können jetzt manuell erfasst werden: optional direkt beim Hinzufügen einer Aktie (nur Bestand) und nachträglich über den ✎-Button, der zum allgemeinen „Aktie bearbeiten"-Modal erweitert wurde — der bisherige Handelsplatz-Wechsel liegt dort jetzt in einem Aufklapper. In Parqet-verbundenen Depots warnt das Modal deutlich, dass der nächste Parqet-Sync manuell geänderte Werte wieder überschreibt. Das P-Badge am Aktiennamen zeigt jetzt die tatsächliche Herkunft an (neues Feld `parqet_synced`): es erscheint nur noch, wenn Einstandskurs/Stückzahl wirklich von Parqet stammen — manuell erfasste oder geänderte Werte tragen kein P. Bestehende Parqet-Positionen erhalten das Badge mit dem nächsten Sync zurück |
| 2.8.13 / 2.13.24 | Aktien-Detail-Modal: Label „P&L" durch „Gewinn & Verlust" ersetzt |
